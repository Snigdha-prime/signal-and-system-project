...
audioFiltered = audioFiltered * 1.1; % Increase the gain factor for amplification
...
% Create notch filters to remove identified noise frequencies
for noiseFreq = noiseFrequencies
    % Design a notch filter around each identified noise frequency
    bw = 15; % Bandwidth of the notch filter (Hz)
    if noiseFreq > 0 % Ensure positive frequency
